...
def total_por_tipo_contagio(): 
    return """select count(c.id_caso) as total, p.nombre as nombre_pais, t.nombre
              from pais p join casos c on (p.codigo = c.codigo_pais)
                          join tipo_contagio t on (t.id_caso_casos = c.id_caso)
              group by nombre_pais, t.nombre
              order by total desc"""
...
def total_por_tipo_recuperacion():
    return """select count(c.id_caso) as total, t.nombre 
              from casos c join tipo_recuperacion t on (t.id_caso_casos = c.id_caso)
              group by t.nombre
              order by total desc
              limit 2"""
